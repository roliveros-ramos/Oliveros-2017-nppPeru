% Original template taken from Steven V. Miller
% Replication files are available on the author's Github account (http://github.com/svmiller)
...
$if(title)$
\title{$title$$if(subtitle)$: $subtitle$$endif$ $if(anonymous)$$else$$if(thanks)$\thanks{$thanks$ -- Version: \today} $endif$$endif$ }
$endif$ 


$if(anonymous)$$else$\author{$for(author)$\Large $author.name$\\ \newline\normalsize\emph{$author.affiliation$}  $sep$ \and $endfor$}$endif$
...
$if(anonymous)$\hfill $else$$for(author)$\textbf{\authorfont $author.name$} \newline \emph{\small $author.affiliation$}  $sep$ \par $endfor$ $endif$
